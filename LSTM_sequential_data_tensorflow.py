...
def run_epoch_eval(session, model, eval_op=None, verbose=False):
  """Runs the model on the given data."""
  start_time = time.time()
  iters = 0
  state = session.run(model._initial_state)
  ...
  with tf.Graph().as_default():
    ...
    with tf.train.SingularMonitoredSession(hooks=hooks, checkpoint_dir=FLAGS.save_path,
      ...
      for i in range(config.max_max_epoch):
        print("Epoch: %d Learning rate: %.5f" % (i + 1, session.run(m._lr)))
        train_loss = run_epoch(session, m, eval_op=m._train_op,
                                     verbose=True)
        print("Epoch: %d Train loss: %.15f" % (i + 1, train_loss))
        valid_loss = run_epoch(session, mvalid)
        print("Epoch: %d Valid loss: %.15f" % (i + 1, valid_loss))

      test_loss, history = run_epoch_eval(session, mtest)
